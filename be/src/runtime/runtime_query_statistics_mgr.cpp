...
int64_t QueryStatisticsCtx::collect_query_statistics(TQueryStatistics* tq_s) {
    ...
    return tmp_qs.get_current_used_memory_bytes();
}
...
void RuntimeQueryStatiticsMgr::report_runtime_query_statistics() {
    ...
    std::map<std::string, std::pair<bool, bool>> qs_status; // <finished, timeout>
    {
        ...
        for (auto& [query_id, qs_ctx_ptr] : _query_statistics_ctx_map) {
            ...
            int64_t curr_mem_bytes = qs_ctx_ptr->collect_query_statistics(&ret_t_qs); // wuy: changed to get curr_mem_bytes
            ...
            LOG(INFO) << "queryid" << query_id << " is_query_finished " << is_query_finished << ", current used memory bytes: "<< curr_mem_bytes;
