...
import org.apache.doris.httpv2.rest.PlanDetailAction;
...
import org.apache.doris.qe.AbstractResultSetMetaData;
import org.apache.doris.qe.ResultSet; // wuy: added
import org.apache.doris.qe.ResultSetMetaData; //wuy: added
...
// import java.sql.ResultSet;  // wuy: deleted
// import java.sql.ResultSetMetaData;  // wuy: deleted
...
public class StmtExecutor {
    ...
    private boolean isExplainDetail = false;
    ...
    public void executeByNereids(TUniqueId queryId) throws Exception {
        if (LOG.isDebugEnabled()) {
    ...
    private void handleQueryStmt() throws Exception {
        ...
        if (queryStmt.isExplain() && queryStmt.getExplainOptions().isDetail()) {
            isExplainDetail = true;
        }
            
        if (queryStmt.isExplain() && !isExplainDetail) {
        ...
        if (!context.getConnectType().equals(ConnectType.ARROW_FLIGHT_SQL)
            ...
            Optional<ResultSet> resultSet = planner.handleQueryInFe(parsedStmt); //wuy: change to AbstractResultSet
    ...
    public void executeAndSendResult(boolean isOutfileQuery, boolean isSendFields,
        ...
        try {
            ...
            if (isExplainDetail) {
                boolean isNereids = planner instanceof NereidsPlanner;
                handleExplainStmt(PlanDetailAction.generatePlanDetailURL(DebugUtil.printId(context.queryId())),
                        isNereids);
                return;
            }
            ...
            if (!isSendFields) {
                if (!isOutfileQuery) {
                    if (ConnectContext.get() != null && isDryRun) {
                        ...
                        ResultSet resultSet = new CommonResultSet(DRY_RUN_QUERY_METADATA,  // wuy: change to AbstractResultSet
    ...
    public void sendResultSet(ResultSet resultSet) throws IOException { // wuy: change ResultSet to AbstractResultSet
        if (context.getConnectType().equals(ConnectType.MYSQL)) {
    ...
    public void handleShowConstraintStmt(List<List<String>> result) throws IOException {
        ShowResultSetMetaData metaData = ShowResultSetMetaData.builder()
                ...
                .build();
        ResultSet resultSet = new ShowResultSet(metaData, result); //wuy: change ResultSet to AbstractResultSet
        sendResultSet(resultSet);
    }
